...
export class RoleService {                                      // Define la clase RoleService que contiene métodos para interactuar con la API de roles.

  private urlApi:string = environment.endpoint + "Role/";       // Define la URL base de la API para las operaciones relacionadas con roles.
  
  constructor(private htt:HttpClient) { }                       // Constructor que inyecta el HttpClient para realizar solicitudes HTTP.

  list():Observable<ResponseApi>{                               // Método para obtener la lista de roles, devuelve un Observable con la respuesta de la API. 
    return this.htt.get<ResponseApi>(`${this.urlApi}List`);     // Realiza una solicitud GET a la API para obtener la lista de roles.
  }
}
